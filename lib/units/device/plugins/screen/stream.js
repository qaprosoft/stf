...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, screenOptions) {
    ...
    FrameProducer.prototype._ensureState = function() {
      log.info("MIRACLE FrameProducer.prototype._ensureState")
      ...
      case FrameProducer.STATE_STOPPED:
        log.info("MIRACLE FrameProducer.STATE_STOPPED")
        if (this.desiredState.next() === FrameProducer.STATE_STARTED) {
          ...
          this._startService().bind(this)
            ...
            .finally(function() {
              log.info("MIRACLE finally _ensureState")
              this._ensureState()
            })
        }
        else {
          log.info("MIRACLE setImmediate(this._ensureState.bind(this))")
          setImmediate(this._ensureState.bind(this))
        }
        break
      case FrameProducer.STATE_STARTED:
        log.info("MIRACLE FrameProducer.STATE_STARTED")
        if (this.desiredState.next() === FrameProducer.STATE_STOPPED) {
          log.info("MIRACLE change to FrameProducer.STATE_STOPPING...")
    ...
    FrameProducer.prototype.restart = function() {
      log.info("MIRACLE FrameProducer.prototype.restart")
      switch (this.runningState) {
      case FrameProducer.STATE_STARTED:
      case FrameProducer.STATE_STARTING:
        this.desiredState.push(FrameProducer.STATE_STOPPED)
    ...
    FrameProducer.prototype.nextFrame = function() {
      log.info("MIRACLE FrameProducer.prototype.nextFrame")
      var frame = null
      var chunk
    ...
    FrameProducer.prototype._failLimitExceeded = function(limit, time) {
      log.info("MIRACLE FrameProducer.prototype._failLimitExceeded")
      this._stop()
      this.failed = true
    ...
    FrameProducer.prototype._readOutput = function(out) {
      log.info("MIRACLE FrameProducer.prototype._readOutput")
    ...
    FrameProducer.prototype._waitForPid = function() {
      log.info("MIRACLE FrameProducer.prototype._waitForPid")
      if (this.pid > 0) {
    ...
    FrameProducer.prototype._connectService = function() {
      log.info("MIRACLE FrameProducer.prototype._connectService")
    ...
    FrameProducer.prototype._stop = function() {
      log.info("MIRACLE FrameProducer.prototype._stop")
      return this._disconnectService(this.socket).bind(this)
        .timeout(2000)
        .then(function() {
          log.info("MIRACLE this._stopService(this.output).timeout(10000)")
