...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, screenOptions) {
    ...
    FrameProducer.prototype._ensureState = function() {
      log.info("MIRACLE FrameProducer.prototype._ensureState: " + this.runningState)
      if (this.desiredState.empty()) {
        log.info("MIRACLE this.desiredState.empty is true!")
        return
      }
      ...
      log.info("MIRACLE this.runningState: " + this.runningState)
      switch (this.runningState) {
      case FrameProducer.STATE_STARTING:
      case FrameProducer.STATE_STOPPING:
        log.info("MIRACLE waiting starting or stopping")
        // Just wait.
        break
      case FrameProducer.STATE_STOPPED:
        log.info("MIRACLE FrameProducer.STATE_STOPPED: " + this.runningState)
        if (this.desiredState.next() === FrameProducer.STATE_STARTED) {
          log.info("MIRACLE change to FrameProducer.STATE_STARTING and binding...")
          this.runningState = FrameProducer.STATE_STARTING
          this._startService().bind(this)
            .then(function(out) {
              log.info("MIRACLE RiskyStream: " + this.runningState)
              this.output = new RiskyStream(out)
                .on('unexpectedEnd', this._outputEnded.bind(this))
              return this._readOutput(this.output.stream)
            })
            .then(function() {
              log.info("MIRACLE _waitForPid: " + this.runningState)
              return this._waitForPid()
            })
            .then(function() {
              log.info("MIRACLE _connectService: " + this.runningState)
              return this._connectService()
            })
            .then(function(socket) {
              log.info("MIRACLE _readBanner: " + this.runningState)
            ...
            .then(function(banner) {
              log.info("MIRACLE _readFrames: " + this.runningState)
              this.banner = banner
              return this._readFrames(this.socket.stream)
            })
            .then(function() {
              this.runningState = FrameProducer.STATE_STARTED
              log.info("MIRACLE change to FrameProducer.STATE_STARTED: " + this.runningState)
              this.emit('start')
            })
            .catch(Promise.CancellationError, function() {
              log.info("MIRACLE stop: this.runningState")
              return this._stop()
            })
            .catch(function(err) {
              log.error("MIRACLE catched: " + err)
            ...
            .finally(function() {
              log.info("MIRACLE finally _ensureState: " + this.runningState)
              this._ensureState()
            })
        }
        else {
          log.info("MIRACLE setImmediate(this._ensureState.bind(this)): " + this.runningState)
          setImmediate(this._ensureState.bind(this))
        }
        break
      case FrameProducer.STATE_STARTED:
        log.info("MIRACLE FrameProducer.STATE_STARTED: " + this.runningState)
        if (this.desiredState.next() === FrameProducer.STATE_STOPPED) {
          log.info("MIRACLE change to FrameProducer.STATE_STOPPING: " + this.runningState)
    ...
    FrameProducer.prototype.start = function() {
      log.info('Requesting frame producer to start: ' + this.runningState)
      this.desiredState.push(FrameProducer.STATE_STARTED)
      this._ensureState()
    }

    FrameProducer.prototype.stop = function() {
      log.info('Requesting frame producer to stop: ' + this.runningState)
      this.desiredState.push(FrameProducer.STATE_STOPPED)
      this._ensureState()
    }

    FrameProducer.prototype.restart = function() {
      log.info("MIRACLE FrameProducer.prototype.restart: " + this.runningState)
      switch (this.runningState) {
      case FrameProducer.STATE_STARTED:
      case FrameProducer.STATE_STARTING:
        log.info("MIRACLE this.desiredState.pushing: " + this.runningState)
        this.desiredState.push(FrameProducer.STATE_STOPPED)
        this.desiredState.push(FrameProducer.STATE_STARTED)
        this._ensureState()
        log.info("MIRACLE this.desiredState.pushed: " + this.runningState)
        break
      }
    }
    ...
    FrameProducer.prototype.nextFrame = function() {
      log.info("MIRACLE FrameProducer.prototype.nextFrame: " + this.runningState)
      var frame = null
      var chunk
    ...
    FrameProducer.prototype._failLimitExceeded = function(limit, time) {
      log.info("MIRACLE FrameProducer.prototype._failLimitExceeded: " + this.runningState)
      this._stop()
      this.failed = true
    ...
    FrameProducer.prototype._readOutput = function(out) {
      log.info("MIRACLE FrameProducer.prototype._readOutput: " + this.runningState)
    ...
    FrameProducer.prototype._waitForPid = function() {
      log.info("MIRACLE FrameProducer.prototype._waitForPid: " + this.runningState)
      if (this.pid > 0) {
    ...
    FrameProducer.prototype._connectService = function() {
      log.info("MIRACLE FrameProducer.prototype._connectService: " + this.runningState)
    ...
    FrameProducer.prototype._stop = function() {
      log.info("MIRACLE FrameProducer.prototype._stop: " + this.runningState)
      return this._disconnectService(this.socket).bind(this)
        .timeout(2000)
        .then(function() {
          log.info("MIRACLE this._stopService(this.output).timeout(10000): " + this.runningState)
          return this._stopService(this.output).timeout(10000)
        })
        .then(function() {
          log.info("MIRACLE stop")
          this.runningState = FrameProducer.STATE_STOPPED
          this.emit('stop')
        })
        .catch(function(err) {
          log.info("MIRACLE err: " + err)
        ...
        .finally(function() {
          log.info("MIRACLE finally: " + this.runningState)
          this.output = null
          this.socket = null
          this.pid = -1
          this.banner = null
    ...
    FrameProducer.prototype._stopService = function(output) {
      ...
      function kindKill() {
        log.info("MIRACLE kindKill")
        return kill('SIGTERM')
      }

      function forceKill() {
        log.info("MIRACLE forceKill")
        return kill('SIGKILL')
      }
