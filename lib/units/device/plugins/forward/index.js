  ...
  .define(function(options, adb, router, push, minirev, group) {
    ...
    function startService() {
      ...
      return adb.shell(options.serial, [
        ...
        .timeout(10000)
        .then(function(out) {
          log.info('MIRACLE then(function(out))')
          ...
          .catch(function(err) {
            if (/closed/.test(err.message) && times > 1) {
              return Promise.delay(delay)
                .then(function() {
                  log.info('MIRACLE tryConnect: ' + times)
    ...
    plugin.createForward = function(id, forward) {
      ...
      return connectService(1)
        .then(function(out) {
          ...
          out.write(header)
          log.info('MIRACLE connectService id: ' + id)
    ...
    plugin.removeForward = function(id) {
      ...
      manager.remove(id)
      log.info('MIRACLE removed reverse port forward id: ' + id)
    ...
    plugin.connect = function(options) {
      ...
      log.info('MIRACLE host: ' + options.targetHost)
      log.info('MIRACLE port: ' + options.targetPort)
