...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group, solo, urlformat) {
    ...
    log.info("MIRACLE activeServer: " + activeServer)

    plugin.port = options.connectPort
    plugin.url = urlformat(options.connectUrlPattern, plugin.port)
    log.info("MIRACLE plugin.url: " + plugin.url)

    plugin.start = function() {
      log.info("MIRACLE plugin.start")
      return new Promise(function(resolve, reject) {
        if (plugin.isRunning()) {
          log.info("MIRACLE plugin already is running...")
          return resolve(plugin.url)
        }

        log.info("MIRACLE plugin.starting")
    ...
    plugin.stop = Promise.method(function() {
      log.info("MIRACLE plugin.stop")
      if (plugin.isRunning()) {
        log.info("MIRACLE stopping")
        activeServer.close()
        activeServer.end()
        activeServer = null
      }
    ...
    plugin.end = Promise.method(function() {
      log.info("MIRACLE plugin.end")
      if (plugin.isRunning()) {
        log.info("MIRACLE ending...")
        activeServer.end()
      }
    })

    plugin.isRunning = function() {
      log.info("MIRACLE isRunning: " + !!activeServer)
      return !!activeServer
    }
