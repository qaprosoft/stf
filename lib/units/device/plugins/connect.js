...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group, solo, urlformat) {
    ...
    log.info("activeServer: " + activeServer)

    plugin.port = options.connectPort
    plugin.url = urlformat(options.connectUrlPattern, plugin.port)
    log.info("plugin.url: " + plugin.url)
    log.info("plugin.port: " + plugin.port)

    plugin.start = function() {
      log.info("plugin.start")
      return new Promise(function(resolve, reject) {
        if (plugin.isRunning()) {
          log.info("plugin already is running...")
    ...
    plugin.isRunning = function() {
      log.info("isRunning: " + !!activeServer)
      return !!activeServer
    }
